...
export default function NewRequestForm() {
  ...
  if (result) {
    return (
      <div className="notice">
        <h2>依頼を投稿しました</h2>
        <p>この管理リンクを保存してください。依頼の終了・削除、申請確認に使います。</p>
        <p><a href={result.manageUrl}>{result.manageUrl}</a></p>
        <p className="warning">このリンクを他人に見せないでください。</p>
        <a className="button" href="/">一覧へ戻る</a>
      </div>
    );
  }
  ...
  return (
    <form className="form" onSubmit={submit}>
      <div><label>依頼タイトル</label><input name="title" required placeholder="例：猫を探しています" /></div>
      <div><label>地域</label><input name="area" required placeholder="例：大阪市北区周辺" /></div>
      <div><label>依頼内容</label><textarea name="body" required placeholder="依頼内容を自由に書いてください" /></div>
      <div className="grid">
        <div><label>報酬・謝礼</label><input name="reward" placeholder="例：3,000円、相談、昼食あり" /></div>
        <div><label>希望日時</label><input name="desired_time" placeholder="例：今週中、土日、相談" /></div>
      </div>
